...
Write-Host "Starting INSFORGE database migration..."

for ($i = 0; $i -lt $migrations.Count; $i++) {
  $sql = $migrations[$i]
  Write-Host "[$($i+1)/$($migrations.Count)] Executing: $sql"
  ...
  try {
    $response = Invoke-RestMethod -Uri "$INSFORGE_URL/rest/v1/rpc/exec_sql" -Method Post -Headers $headers -Body $body -ErrorAction Stop
    Write-Host "  ✅ Success"
  } catch {
